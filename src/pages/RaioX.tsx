...
//   5. A página nunca fica muda: se ele pular uma pergunta, ela avisa.
//
// Sobre o "saber mais" (ideia do Thiago, 02/09): quando ele escolhe entre
// duas dores, escolhe a que dói MAIS — o outro lado quase sempre também é
...
// Domínio onde o app vive. No subdomínio raiox.* o cadastro TEM que sair pra
// cá: a sessão do Supabase é por origem, então quem se cadastrasse no
// raiox.5gobra.com.br voltaria ao app aparentando estar deslogado.
// A query vai junto pra não perder a UTM no meio do caminho.
...
type Zoom = { titulo: string; tela: React.ReactNode }
...
function Pergunta({
  ...
  return (
    ...
    >
      ...
      {feito && (
        <span className="absolute -top-2.5 right-4 bg-status-andamento text-white text-[11px] font-bold w-[22px] h-[22px] rounded-full grid place-items-center leading-none">
          ✓
        </span>
      )}
...
/** Resumo do lado que ele NÃO escolheu, com o botão que abre o bloco inteiro. */
function Tambem({
  resumo,
  aberto,
  aoAbrir,
  ...
  return (
    <div className="mt-3 bg-slate-50 border border-dashed border-slate-300 rounded-xl p-3.5">
      ...
      <p className="text-[15.5px] text-slate-600 leading-snug">{resumo}</p>
      {!aberto && (
        <button type="button" onClick={aoAbrir} className="btn-ghost w-full mt-3.5">
...
/** Vista explodida: balões numerados na tela + legenda embaixo. */
function Explodida({
  rotulo,
  tela,
  pinos,
  ...
  aoAmpliar: () => void
  /** limita a caixa dos balões à largura da arte (o celular do técnico é
      estreito; sem isso os balões ficam boiando no vazio no desktop). */
  larguraMax?: number
}) {
  return (
...
/* ==================== os quatro blocos de solução ====================
   Cada um vive num componente próprio porque aparece em DOIS lugares:
   quando ele escolhe aquela dor, e quando ele abre o "saber mais" do
   lado que não escolheu. */
  ...
  return (
    <Resposta>
      ...
      <p className="text-slate-600 text-[16.5px]">
        Seu cliente acha que liberou. Mas quem libera é o responsável da obra — e se o requadro, o
        contramarco ou a soleira não estão prontos, o vão não está liberado e a contagem não começa.
        Na hora da cobrança, a conta sobra pra você.
      </p>
      ...
      <div className="mt-3.5 border border-slate-200 rounded-2xl bg-white overflow-hidden">
        <div className="flex items-center justify-between gap-2.5 px-3.5 py-2.5 border-b border-slate-200 bg-slate-50">
          ...
          <div className="flex gap-1.5">
            {[1, 2, 3].map((n) => (
              <span key={n} className={'w-[7px] h-[7px] rounded-full ' + (n === passo ? 'bg-laranja' : 'bg-slate-300')} />
            ))}
          </div>
        </div>
        <div className="p-3.5">
          {passo === 1 && (
            <>
              <p className="text-[15.5px] text-slate-600 mb-3">
                <b className="text-slate-900">1. Ele abre o link no celular</b> — sem baixar app, sem
                criar senha — e vê a obra dele peça por peça.
              </p>
              <TelaCliente />
            </>
          )}
          {passo === 2 && (
            <>
              <p className="text-[15.5px] text-slate-600 mb-3">
                <b className="text-slate-900">2. Apareceu problema no vão.</b> O técnico registra com
                foto, e a bola passa pro cliente — com data e hora.
              </p>
              <TelaHistorico />
            </>
          )}
          {passo === 3 && (
            <>
              <p className="text-[15.5px] text-slate-600 mb-3">
                <b className="text-slate-900">3. No fim, o aceite é dado dentro do sistema</b> — e
                tudo isso vira um dossiê em PDF.
              </p>
              <TelaAceite />
            </>
          )}
        ...
        <div className="px-3.5 pb-3.5 flex gap-2.5">
          ...
          <button type="button" onClick={() => setPasso(passo === 3 ? 1 : passo + 1)} className="btn-primary flex-1">
            {passo === 3 ? 'Ver de novo' : 'Próximo passo'}
          </button>
        </div>
      </div>

      <Frase>Da próxima vez que ele jurar que combinou outra coisa, você não discute. Você abre.</Frase>
    </Resposta>
  )
}

function BlocoEquipe({ aoAmpliar }: { aoAmpliar: (z: Zoom) => void }) {
  return (
    <Resposta>
      ...
      <p className="text-slate-900 text-[16.5px] font-semibold mt-2.5">
        No G Obra o técnico registra no card, na hora, com foto. E o registro é o trabalho dele, não
        uma tarefa a mais.
      </p>
      <Explodida
        ...
        larguraMax={244}
        tela={<TelaTecnico />}
        aoAmpliar={() => aoAmpliar({ titulo: 'Link do técnico', tela: <TelaTecnico /> })}
        pinos={[
...
function BlocoPrazo({ aoAmpliar }: { aoAmpliar: (z: Zoom) => void }) {
  return (
    <Resposta>
      <Titulo3>O prazo deixa de ser promessa e vira conta</Titulo3>
      <p className="text-slate-600 text-[16.5px]">
        Você define quantos dias cada etapa leva, contando a partir do gatilho que importa: a
        liberação do vão, o contramarco, a medição aprovada.
      </p>
      <p className="text-slate-600 text-[16.5px] mt-2.5">
        Daí o sistema conta sozinho. Cada obra mostra em que fase está e quantos dias faltam — e
        quando o prazo aperta, aparece na tela <b className="text-slate-900">antes</b> de apertar de
        verdade.
      </p>
      <Explodida
        rotulo="Cronograma por gatilho — vista explodida"
        tela={<TelaCronograma />}
        aoAmpliar={() => aoAmpliar({ titulo: 'Cronograma', tela: <TelaCronograma /> })}
        pinos={[
...
function BlocoMetas({ aoAmpliar }: { aoAmpliar: (z: Zoom) => void }) {
  return (
    <Resposta>
      ...
      <p className="text-slate-600 text-[16.5px] mt-2.5">
        Vira um jogo que a fábrica joga junto: quanto já foi feito no mês, o que falta pra bater, quem
        está na frente.
      </p>
      <Explodida
        rotulo="Tela de metas — vista explodida"
        tela={<TelaMetas />}
        aoAmpliar={() => aoAmpliar({ titulo: 'Metas', tela: <TelaMetas /> })}
        pinos={[
...
const RESUMO_CLIENTE = (
  <>
    <b className="text-slate-900">O outro lado disso:</b> cada obra tem um link só dela pro seu
    cliente. Ele acompanha o estágio de cada peça sem te ligar, os problemas do vão ficam
    registrados com foto e data, e o aceite final é dado ali dentro — virando um dossiê em PDF.
  </>
)
const RESUMO_EQUIPE = (
  <>
    <b className="text-slate-900">O outro lado disso:</b> o técnico entra por um link no celular e
    registra a medição na hora, com foto. Você não recebe relatório no fim do dia — você vê o
    movimento aparecendo na sua tela enquanto acontece.
  </>
)
const RESUMO_PRAZO = (
  <>
    <b className="text-slate-900">O outro lado disso:</b> o cronograma conta o prazo de cada etapa a
    partir do gatilho certo — a liberação do vão, a medição aprovada — e mostra o atraso em dias,
    antes do cliente cobrar.
  </>
)
const RESUMO_METAS = (
  <>
    <b className="text-slate-900">O outro lado disso:</b> a tela de metas mostra quanto a equipe já
    produziu no mês, sempre atualizada. Cada um vê onde está e quanto falta — deixa de ser você
    cobrando, passa a ser o placar.
  </>
)
...
export default function RaioX() {
  ...
  const [abriu2, setAbriu2] = useState(false)   // "saber mais" do outro lado
  const [abriu3, setAbriu3] = useState(false)
  const [passoSim, setPassoSim] = useState(1)
  const [passoSim2, setPassoSim2] = useState(1) // simulação do bloco aberto pelo "saber mais"
    ...
    <div className="min-h-full bg-[#f4f6fa] text-slate-900">
      ...
      <div className="max-w-[620px] mx-auto px-4 pb-32">
        ...
        <section className="mt-7 grid gap-3.5">
          <Pergunta id="card-q1" titulo="Quantas obras você administra hoje?" apoio="Contando as que estão rodando e os contratos fechados esperando." opcoes={P1} valor={resp.q1} vez={vez === 'q1'} aoEscolher={(o) => responder('q1', o)} />
          <Pergunta id="card-q2" titulo="Como você controla isso hoje?" opcoes={P2} valor={resp.q2} vez={vez === 'q2'} aoEscolher={(o) => responder('q2', o)} />
          <Pergunta id="card-q3" titulo="Quanto tempo você leva pra saber, com certeza, em que fase está uma obra?" opcoes={P3} valor={resp.q3} vez={vez === 'q3'} colunas={1} aoEscolher={(o) => responder('q3', o)} />
        </section>
          ...
          <section id="rv1" className="mt-4">
            ...
            {extras.cabeca && (
              <div className="mt-3 bg-slate-50 border border-dashed border-slate-300 rounded-xl p-3.5">
                ...
                <p className="text-[15.5px] text-slate-600 leading-snug">
                  <b className="text-slate-900">Se está tudo com você, a fábrica anda no seu ritmo — e para quando você para.</b>{' '}
          ...
          <section className="mt-7">
            ...
            {ramo2 && (
              <div id="rv2" className="mt-4">
                {ramo2 === 'cliente'
                  ? <BlocoCliente passo={passoSim} setPasso={setPassoSim} />
                  : <BlocoEquipe aoAmpliar={setZoom} />}

                <Tambem
                  resumo={ramo2 === 'cliente' ? RESUMO_EQUIPE : RESUMO_CLIENTE}
                  aberto={abriu2}
                  aoAbrir={() => {
                    setAbriu2(true)
                    trackCustom('raiox_saber_mais', { bloco: ramo2 === 'cliente' ? 'equipe' : 'cliente' })
                    rolarAte('mais2')
                  }}
                />

                {abriu2 && (
                  <div id="mais2" className="mt-3">
                    {ramo2 === 'cliente'
                      ? <BlocoEquipe aoAmpliar={setZoom} />
                      : <BlocoCliente passo={passoSim2} setPasso={setPassoSim2} />}
                  </div>
                )}

                <Proxima texto="Mais 3 perguntas." alvo="card-q5" aoIr={rolarAte} />
              </div>
            )}
          </section>
        )}
          ...
          <section className="mt-7">
            ...
            {ramo3 && (
              <div id="rv3" className="mt-4">
                {ramo3 === 'prazo' ? <BlocoPrazo aoAmpliar={setZoom} /> : <BlocoMetas aoAmpliar={setZoom} />}

                <Tambem
                  resumo={ramo3 === 'prazo' ? RESUMO_METAS : RESUMO_PRAZO}
                  aberto={abriu3}
                  aoAbrir={() => {
                    setAbriu3(true)
                    trackCustom('raiox_saber_mais', { bloco: ramo3 === 'prazo' ? 'metas' : 'prazo' })
                    rolarAte('mais3')
                  }}
                />

                {abriu3 && (
                  <div id="mais3" className="mt-3">
                    {ramo3 === 'prazo' ? <BlocoMetas aoAmpliar={setZoom} /> : <BlocoPrazo aoAmpliar={setZoom} />}
                  </div>
                )}

                <Proxima texto="Últimas 2 perguntas — e são as que mais doem." alvo="card-q6" aoIr={rolarAte} />
              </div>
            )}
          </section>
        )}
        ...
        {ramo3 && (
          <section className="mt-7 grid gap-3.5">
            <Pergunta id="card-q6" titulo="Mês passado, quantas peças você refez — por erro de medida ou por vão que não estava pronto?" opcoes={P6} valor={resp.q6} vez={vez === 'q6'} aoEscolher={(o) => responder('q6', o)} />
            <Pergunta id="card-q7" titulo="E quanto tempo você gastou explicando de novo a mesma coisa — pro funcionário, pro cliente, pro técnico?" opcoes={P7} valor={resp.q7} vez={vez === 'q7'} aoEscolher={(o) => responder('q7', o)} />
          </section>
        )}
          ...
          <section id="rv4" className="mt-4">
            ...
            {!redondo ? (
              ...
              <div className="mt-3.5 bg-slate-50 border border-slate-200 rounded-2xl p-5">
                <span className="block font-mono text-[10.5px] tracking-[.12em] uppercase text-slate-400 mb-3">então tá redondo aí</span>
                <p className="text-[16.5px] text-slate-600">Se você não refez peça e não perdeu tempo explicando de novo, sua gestão está funcionando. Sério.</p>
                <p className="text-[16.5px] text-slate-600 mt-2.5">A pergunta é outra: ela continua funcionando quando você dobrar de obra? O teste é grátis por 14 dias e não pede cartão — dá pra descobrir sem apostar nada.</p>
                <span className="block font-display font-bold text-[21px] text-slate-900 leading-snug mt-3.5">R$ {PRECO} por mês. Menos que uma peça refeita.</span>
              </div>
            )}
          </section>
        )}

        {/* ---------- fechamento: IMEDIATAMENTE depois da última resposta ---------- */}
        {tela4Pronta && (
          <section id="fechamento" className="mt-8">
            <h2 className="font-display font-extrabold text-[26px] leading-tight">Você viu o que o G Obra faz. Como quer seguir?</h2>
            ...
            <div className="relative border-2 border-laranja rounded-2xl bg-white p-[18px] mt-3 shadow-[0_8px_26px_rgba(255,106,0,.16)]">
              ...
              <button type="button" onClick={abrirCheckout} className="btn-primary w-full">Contratar e agendar a implementação</button>
            </div>
            ...
            <div className="border border-slate-200 rounded-2xl bg-white p-[18px] mt-3">
              ...
              <a href={linkWhats()} target="_blank" rel="noopener noreferrer" onClick={() => { trackContact('cta-final'); trackCustom('raiox_whatsapp') }} className="btn-ghost w-full mt-3.5">
                Chamar no WhatsApp
              </a>
            </div>
          </section>
          ...
          <>
            <section id="reasseguro" className="mt-9">
              <div className="bg-white border border-slate-200 rounded-2xl p-5">
                ...
                <p className="text-[16.5px] text-slate-600 mt-2">O resto do sistema é assim também: quem abre pela primeira vez consegue navegar sem manual.</p>
              </div>

              <div className="grid sm:grid-cols-2 gap-3 mt-3.5">
                <div className="rounded-2xl p-4 border border-slate-200 bg-slate-100">
                  <h4 className="font-mono text-[10.5px] tracking-[.12em] uppercase text-slate-400 font-medium m-0 mb-2.5">Como é hoje aí</h4>
                ...
                <div className="rounded-2xl p-4 border border-laranja-border bg-laranja-soft">
                  <h4 className="font-mono text-[10.5px] tracking-[.12em] uppercase text-laranja-dark font-medium m-0 mb-2.5">Como fica</h4>
                  <ul className="list-none m-0 p-0 grid gap-2">
                    {[

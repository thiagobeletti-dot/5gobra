...
type ModalGlobal = 'nenhum' | 'novo' | 'importar' | 'tecnicos' | 'documentos' | 'editar-obra'
...
export default function Obra() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[220px_1fr] min-h-screen">
        ...
        <div className="bg-white border-b border-slate-200 px-4 md:px-7 py-3.5 flex items-center gap-3 md:gap-5">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2.5 font-bold text-[15px]">
              <span>{dados.obra.nome}</span>
              {data.modo === 'banco' && data.obraReal && (
                <button
                  onClick={() => setModalGlobal('editar-obra')}
                  title="Editar dados da obra"
                  aria-label="Editar dados da obra"
      ...
      {modalGlobal === 'editar-obra' && data.obraReal && (
        <ModalEditarObra
          obra={data.obraReal}
          onClose={() => setModalGlobal('nenhum')}
          onSalvo={() => {
            setModalGlobal('nenhum')
            // Recarrega a página pra refletir os novos dados em toda a UI.
            // Quick win — futuro: refresh otimista via hook.
            window.location.reload()
          }}
        />
      )}

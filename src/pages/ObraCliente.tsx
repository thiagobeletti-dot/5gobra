...
function CardClienteView({ card, onClick }: { card: Card; onClick: () => void }) {
  ...
  const subStatusInterno = card.subStatus && (
    ...
    card.subStatus === 'Tipologia não executável'
  )
  const subClienteVisivel = card.subStatus && !subStatusInterno ? card.subStatus : null
  const labelStatus =
    subClienteVisivel
    ? subClienteVisivel
...
function ModalCardCliente({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm grid place-items-end md:place-items-center p-0 md:p-5 z-40" onClick={onClose} role="dialog" aria-modal="true">
      <div className="bg-white border border-slate-200 rounded-t-2xl md:rounded-2xl w-full max-w-2xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-5 md:px-6 py-4 md:py-5 space-y-5">
          ...
          {card.aba === 'conclusao' && (
            <div>
              {card.encerrado ? (
                // Card encerrado (acordo, apontamento ou cancelamento) — não há
                // aceite a confirmar. Mostra o motivo do encerramento.
                <div className="bg-slate-100 border border-slate-300 px-4 py-3 rounded-lg text-xs text-slate-700">
                  ...
                  <p className="text-slate-500">Veja o histórico completo abaixo.</p>
                </div>
              ) : card.aceiteFinal ? (
